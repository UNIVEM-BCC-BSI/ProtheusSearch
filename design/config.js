...
// !!! Função resposanvel por montar o HTML de cada linha da tabela
const renderTableRow = (tbody, item) => {
...
// !!! Função responsável por tratar os dados recebidos do API
const populateTable = (data) => {
  const table = document.getElementById("data-table");

  // !!!  Codigo para substituir o while que era usado para limpar a tabela
  const tbody = document.getElementById("table-content");
  if (tbody) table.removeChild(tbody);

  // !!! Cria novo element tbody para ser populado com os novos dados
  const newTbody = document.createElement("tbody");
  newTbody.id = "table-content";
  table.appendChild(newTbody);

  // !!!  Codigo estava limpando também o thead da tabela, além de apresentar má performance com muitos itens
  ...
  data.forEach((item) => renderTableRow(newTbody, item));
};
...
const fetchData = async () => {
  ...
  // !!! Alterado a forma de buscar o itens da API para async e wait ao invés de then e catch
...
const handleDefaultDataTable = async () => {
  const data = await fetchData();
  populateTable(data);
};
handleDefaultDataTable();
...
const handleFilter = async () => {
  ...
  const filteredData = data.filter((d) => d.codigo.includes(filterValue) || d.descricao.includes(filterValue));
...
document.addEventListener("click", (event) => {
  ...
  const isSuggestionList =
    event.target === suggestionList || suggestionList.contains(event.target);
